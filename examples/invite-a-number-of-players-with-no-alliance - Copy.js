...
//To invite a number of players who have no alliance and are over a certain level:
...
var minLevel=400;   //min level to invite from
$.get(
    ...
    function (data) {
        ...
        for (var i = startFrom; i < startFrom + inviteCount; i++) {
            if(!data.data.players[data.data.unallied[i]])
                continue;
            if(data.data.players[data.data.unallied[i]].points<minLevel){
                inviteCount++;
                continue;
            }
            console.log('inviting '+data.data.players[data.data.unallied[i]].name+' - '+data.data.players[data.data.unallied[i]].points);
            $.post(
